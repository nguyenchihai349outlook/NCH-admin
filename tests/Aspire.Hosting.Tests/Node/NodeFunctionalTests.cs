...
public class NodeFunctionalTests
{
    ...
    public async Task VerifyNodeAppWorks()
    {
        ...
        using var nodeClient = testProgram.App!.CreateHttpClient(testProgram.NodeAppBuilder!, "http");
        var response0 = await nodeClient.GetStringAsync("/", cts.Token);

        using var npmClient = testProgram.App!.CreateHttpClient(testProgram.NodeAppBuilder!, "http");
